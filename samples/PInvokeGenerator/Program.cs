...
");
		...
		abstract class Named
		{
			...
			public abstract void Write (TextWriter w, string typeScope);
		}
		...
		class Struct : Named
		{
			...
			{
				if (IsEnum) {
					w.WriteLine ("{4} enum {0} // {1} ({2}, {3})", string.IsNullOrEmpty (Name) ? "_" + new string (Path.GetFileNameWithoutExtension (SourceFileName).TakeWhile (c => char.IsLetterOrDigit (c)).ToArray ()) + "_" + Line + "_" + Column : Name, SourceFileName, Line, Column, typeScope);
					w.WriteLine("{");
					...
					foreach (var m in Fields) {
						...
						w.WriteLine ("\t{2}{3} {0} @{1};", m.Type, string.IsNullOrEmpty (m.Name) ? "_" + Fields.IndexOf (m) : m.Name, m.TypeDetails, typeScope);
					}
